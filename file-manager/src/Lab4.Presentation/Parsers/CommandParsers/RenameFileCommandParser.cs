...
public sealed class RenameFileCommandParser : ICommandParser
{
    private readonly IReadOnlyList<IPositionalApplier<RenameFileCommandBuilder>> _positionalAppliers;

    private readonly IFlagApplierLink<RenameFileCommandBuilder>? _flagApplierChain;

    public RenameFileCommandParser(
        IReadOnlyList<IPositionalApplier<RenameFileCommandBuilder>> positionalAppliers,
        IFlagApplierLink<RenameFileCommandBuilder>? applierChain)
    {
    ...
    private IParsingError? ParsePositional(RenameFileCommandBuilder builder, string[] tokens, ref int currentIndex)
    {
        foreach (IPositionalApplier<RenameFileCommandBuilder> positionalArgument
